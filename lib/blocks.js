...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.alarmxd.getBlockHeader(hash, function(err, info) {
		if (err) {
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.alarmxd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.alarmxd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.alarmxd.height - blockCached.height + 1;
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.alarmxd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.alarmxd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
      ...
      self.node.services.alarmxd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.alarmxd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.alarmxd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.alarmxd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  var nBudgetPaymentsStartBlock = 2100000000;
  var nSubsidyBase;

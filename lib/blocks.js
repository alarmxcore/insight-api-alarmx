...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        //Genesis reward is 2000000 (Premine 2%).
        cb(null, parseFloat("1000000".toString(10)).toFixed(8));
    }
    else{
	    ...
	    // yearly decline of production by ~9.09% per year, projected ~100M coins max by year 2038+.

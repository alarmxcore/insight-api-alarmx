...
GovObjectController.prototype.govObjectList = function(options, callback) {
    this.node.services.alarmxd.govObjectList(options, function(err, result) {
        if (err) {
            return callback(err);
        }
...
GovObjectController.prototype.getHash = function(hash, callback) {
    ...
    this.node.services.alarmxd.govObjectHash(hash, function(err, result) {
        if (err) {
            return callback(err);
        }
...
GovObjectController.prototype.govObjectCheck = function(req, res) {
  ...
  this.node.services.alarmxd.govObjectCheck(hexdata, function(err, result) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(result);
